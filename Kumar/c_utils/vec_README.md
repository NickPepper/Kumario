...
When you are done with the vector the `vec_free()` function should be called
...
| float           | vec_float_t   |
| double          | vec_double_t  |
| unsigned int    | vec_uint_t    |

To define a new vector type the `vec_t()` macro should be used:
```c
/* Creates the type vec_ulong_t for storing unsigned longs */
typedef vec_t(unsigned long) vec_ulong_t;
```
...
```c
/* Typedefs the struct `vec_t_fp` as a container for type FILE* */
typedef vec_t(FILE*) vec_t_fp;
```
